...
class PolicyNetwork {
  ...
  protected policyNet: tf.Sequential;
  private currentActions_: Float32Array;
   ...
   */
  constructor(hiddenLayerSizesOrModel: number[] | tf.Sequential) {
    if (hiddenLayerSizesOrModel instanceof tf.Sequential) {
   ...
   */
  createPolicyNetwork(hiddenLayerSizes: number[]) {
  ...
  getGradientsAndSaveActions(inputTensor) {
    const f = () => tf.tidy(() => {
      ...
      const labels =
          tf.sub(1, tf.tensor2d(this.currentActions_, actions.shape as [number, number]));
      return tf.losses.sigmoidCrossEntropy(labels, logits).asScalar();
    });
    return tf.variableGrads(f);
  }

  getCurrentActions(): Float32Array {
   ...
   */
  getLogitsAndActions(inputs: tf.Tensor): tf.Tensor[] {
    return tf.tidy(() => {
      ...
      const leftRightProbs = tf.concat([leftProb, tf.sub(1, leftProb)], 1) as tf.Tensor1D;
   ...
   *   `batchSize`.
   */
  getActions(inputs): Float32Array {
    return this.getLogitsAndActions(inputs)[1].dataSync() as Float32Array;
  }
...
export class SaveablePolicyNetwork extends PolicyNetwork {
   ...
   *
   * @param {number[]} hiddenLayerSizes
   */
  constructor(hiddenLayerSizes: number[]) {
